...
import { getLayoutVersion } from './layout_ver';
...
export const _MARKET_STAT_LAYOUT_V1 = struct([
  ...
  export class Market {
    ...
    dump() {
      return {
        "base" : this.decoded.baseMint.toBase58(),
        "quote" : this.decoded.quoteMint.toBase58(),
      }
    }
    ...
    async loadOrdersForOwner(
      ...
      const open = this.filterForOpenOrders(bids, asks, openOrdersAccounts);
    ...
    async getTokenAccountsByOwnerForMint(
      ...
      //console.log("getTokenAccountsByOwnerForMint ", mintAddress.toBase58()," owner ", ownerAddress.toBase58());
      return (
    ...
    async findQuoteTokenAccountsForOwner(
      ...
      if (this.quoteMintAddress.equals(WRAPPED_SAFE_MINT) && includeUnwrappedSol) {
        const [wrapped, unwrapped] = await Promise.all([
    ...
    //     const  orderIx = DexInstructions.newOrderV3({
    //         market: this.address,
    ...
    public async findBestFeeDiscountKey(
      connection: Connection,
      ownerAddress: PublicKey,
    ...
    public async createOpenOrders(connection: Connection, params: { owner : Keypair, account : Keypair}) {
        // create the account and initialize in one transaction
        ...
        const init_ix = DexInstructions.initOpenOrders({
            market:this.address,
            openOrders:params.account.publicKey,
        ...
        const signers : Keypair[]= [ params.owner, params.account];
        return await connection.sendTransaction(tx,signers);
    }
    public async createOpenOrdersWithWallet(connection: Connection, params: { owner : Wallet, account : Keypair}) {
      // create the account and initialize in one transaction
      const ix = await OpenOrders.makeCreateAccountTransaction(
          connection,
          this.address,
      ...
      const signers : Keypair[]= [ params.account];
      return await this._sendTransaction(connection,tx,signers);
  }
    ...
    makePlaceOrderInstruction<T extends PublicKey | Keypair>(
      ...
      // @ts-ignore
      const ownerAddress: PublicKey = owner.publicKey ?? owner;
      console.log("mpoi ownerAddr : ", ownerAddress.toBase58());
    ...
    makeNewOrderV3Instruction<T extends PublicKey | Keypair>(
      ...
      // @ts-ignore
      const ownerAddress: PublicKey = owner.publicKey ?? owner;
      console.log("order v3 signer : ", ownerAddress.toBase58());
    ...
    private async _sendTransaction(
      ...
      let accounts = transaction.instructions.keys();
      console.log("looping");
      let a;
      for( a in accounts.next()){
        console.log(accounts[a])
      }
      console.log("accounts: ",transaction.instructions.keys());

      //console.log("accounts: ",(transaction.instructions.keys as PublicKey[]).map((k)=>(k.toBase58())));
    ...
    _allOpenOrders() : OpenOrders[] {
        let oOrders : OpenOrders[] = [];
        const cache : {} = this._openOrdersAccountsCache;

        for (const [k, val] of Object.entries(cache)) {
            let accounts : OpenOrders[]= ((val as any).accounts as OpenOrders[]);
    ...
    public async consumeEvents(connection : Connection,payer: Keypair){
          let ooKeys = this._allOpenOrders().map( oo => oo.address);
          return this._consumeEvents(connection,ooKeys,payer);
    }

    async _consumeEvents(connection: Connection, ooAccounts: PublicKey[], payer: Keypair) {
        
        const ceIx = this.makeConsumeEventsInstruction(ooAccounts,ooAccounts.length);
        const tx = new Transaction();
        tx.add(ceIx);
    ...
    public async settleFunds(connection: Connection, 
        owner : Keypair,
        baseWallet: PublicKey,
        quoteWallet: PublicKey
        ) {
        let oOrders = this._allOpenOrders();
        const filtered = oOrders.filter((oOrder) => (oOrder.owner.toBase58() == owner.publicKey.toBase58()));

        console.log(filtered.length)
        return this._settleFunds(connection,owner,
            filtered[0],
            baseWallet,
            quoteWallet );
    }
  ...
  export class OpenOrders {
    ...
    static async makeCreateAccountTransaction(
      ...
      const span = this.getLayout(programId).span
 ...
 static async makeCreateOOAccountTransaction(
  connection: Connection,
  marketAddress: PublicKey,
  ownerAddress: PublicKey,
  programId: PublicKey,
  newAccountAddress: PublicKey | null,
) : Promise<Transaction> {
  console.log("makeOO signer : ", ownerAddress.toBase58());
  const span = this.getLayout(programId).span
  const t = new Transaction()
  const naAddr = newAccountAddress ? newAccountAddress : Keypair.generate().publicKey;
  console.log("OOAccount pubkey: ",naAddr.toBase58());
  const ca_ix = SystemProgram.createAccount({
    fromPubkey: ownerAddress,
    newAccountPubkey: naAddr,
    lamports: await connection.getMinimumBalanceForRentExemption(
        span,
    ),
    space: span,
