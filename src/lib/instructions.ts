...
export class DexInstructions {
  ...
  static newOrderV3({
    ...
    const keys = [
      ...
      { pubkey: asks, isSigner: false, isWritable: true },
      { pubkey: payer, isSigner: false, isWritable: true },
      { pubkey: owner, isSigner: true, isWritable: false }, // 7
      { pubkey: baseVault, isSigner: false, isWritable: true },
      { pubkey: quoteVault, isSigner: false, isWritable: true },
      { pubkey: TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
      { pubkey: SYSVAR_RENT_PUBKEY, isSigner: false, isWritable: false },
    ];
    console.log(keys.map((v)=>(v.pubkey.toBase58())))
  ...
  static consumeEvents({
    ...
    console.log("consume events oo keys count: ", openOrdersAccounts.length)
    const keys = openOrdersAccounts.map((key) => ({ pubkey: key, isSigner: false, isWritable: true } ));
    keys.push({ pubkey: market, isSigner: false, isWritable: true } );
    keys.push({ pubkey: eventQueue, isSigner: false, isWritable: true  });
    keys.push({ pubkey: coinFee, isSigner: false, isWritable: true } );
    keys.push({ pubkey: pcFee, isSigner: false, isWritable: true } );
    console.log("consume events keys count: ", keys.length)
