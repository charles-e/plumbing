...
(async() => {
    ...
    console.log("w1: key: %s oo: %s base: %s quote %s ",w1.publicKey,w1_info.orders,w1_info.base,w1_info.quote);
    console.log("w2: key: %s oo: %s base: %s quote %s ",w2.publicKey,w2_info.orders,w2_info.base,w2_info.quote);
  ...
  const all = market._allOpenOrders();
  console.log(all.map((o) => (o.owner.toBase58())));

  const filtered = all.filter((oOrder) => (oOrder.owner.toBase58() == w1.publicKey.toBase58()));
  console.log(filtered.length);
  const filtered2 = all.filter((oOrder) => (oOrder.owner.toBase58() == w2.publicKey.toBase58()));
  console.log(filtered.length)
;
